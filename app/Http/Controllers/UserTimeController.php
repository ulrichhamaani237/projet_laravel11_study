...
class UserTimeController extends Controller
{
    ...
    {
        // dd($request->all());
        User_time::select('user_id', '=', Auth::user()->id)->delete();

        if (!empty($request->week)) {
            foreach ($request->week as $value) {
                if (!empty($value['status'])) {
                    
                    $record = new User_time;
        ...
        return redirect('admin/schedule')->with('success', 'Schedule update successful..!');
    }
}
